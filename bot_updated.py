...
async def kvkopponent(
    ...
    if compare:
        ...
        for idx, (our_row, our_detail, opp_row, opp_detail) in enumerate(detail_rows, 1):
            ...
            # Keep the comparison metrics directly under each player. This is
            # deliberately a narrow, repeated-label format that reads well on
            # Discord mobile and does not depend on fixed-width columns.
            for label, key in (
                ("Hero Power", "Hero Power"),
                ("Research", "Research"),
                ("Gov. Gear", "Gov. Gear"),
                ...
                ov = ours_metric_values[key]
                rv = opp_metric_values[key]
                if ov is not None and rv is not None:
                    ...
                    ow = rw = False
                block.append(metric_line(label, ov, ow))
            ...
            for label, key in (
                ...
                if ov is not None and rv is not None:
                    ...
                    rw = rv is not None and ov is None
                block.append(metric_line(label, rv, rw))

            block.extend(["", *hero_block(our_detail, "810"), "", *hero_block(opp_detail, str(kingdom))])
